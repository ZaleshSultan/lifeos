...
A previous attempt at this exact task went off-script: instead of building
what's specified below, it made unrequested changes to the already-shipped,
already-verified Academic Engine Phase 1 schema (`study_courses` /
`course_schedules` / `assessment_items`) - new columns, new constraints, a
new migration file, `REVOKE` statements - none of which were asked for, and
it never touched the actual assigned task at all.

Hard rule for this task: **you will not create, edit, or touch anything
under `supabase/migrations/` in this task.** This phase needs zero new
tables, zero new columns, zero new constraints on the existing academic
schema. If partway through you find yourself thinking "I should also fix/
add X to the existing schema" - stop, do not do it, and instead mention it
as a note in your final summary for a human to decide on separately. Every
file you touch must be directly required to build the three new endpoints
and their auth below - nothing adjacent, nothing "while I'm in here."
...
Windows machine). This phase does NOT touch Agata's codebase at all - that
repo isn't even part of this workspace. This phase only builds the
LifeOS-side surface Agata will eventually call over HTTP.
...
per-course methods). This is a new TypeScript method using the EXISTING
schema exactly as it already is - it is not a reason to alter any table,
...
"need a new cross-course store method, not a schema change" note as above.
...
- **Any change under `supabase/migrations/` - no new migration file, no
  edits to any existing one.** Repeated from the top because this is
  exactly what went wrong last time.
- Anything in the actual Agata (Python/Windows) codebase - that repo isn't
  part of this workspace at all.
...
threading for `lifeosAgataApiKey`, and unit tests. Nothing else - not even
a well-intentioned schema fix. If something in this spec turns out to be
wrong once you actually look at the current codebase (e.g. `tmaData()`
doesn't shape the way assumed here, or a relevant helper already exists
under a different name), follow what's actually there, work around it with
TypeScript-only changes, and note the discrepancy in your summary rather
than guessing silently or reaching for a schema migration to "fix" it.

Before you finish, do a final self-check: run `git diff --stat` (or
equivalent) and confirm every changed file is one of: a route handler file,
`packages/db/src/lifeos-store.ts`, `packages/db/src/types.ts`, or a test
file for those. If `supabase/migrations/` appears anywhere in that diff,
that's a signal you've gone off-task - revert it before finishing.
