...
def render_expense(entity: JsonObject, _detail: JsonObject | None) -> RenderedNote:
    title = entity.get("title") or "Expense"
    ...
    markdown = "\n\n".join(
        [
            frontmatter(entity, {"kind": "expense"}),
            f"# Expense: {title}",
            md_table(
                [
                    ("Amount", amount),
    ...
    return RenderedNote(["Finance", "Expenses", f"{created} - {title} - {short_id(entity.get('id'))}.md"], markdown)
