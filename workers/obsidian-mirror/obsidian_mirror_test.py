...
class ObsidianMirrorRoutingTest(unittest.TestCase):
    ...
    def connected_settings(self, vault: Path) -> dict[str, object]:
        return {
            "enabled": True,
            "mode": "local_vault",
    ...
    def test_user_a_queue_item_uses_user_a_vault_path(self) -> None:
        with tempfile.TemporaryDirectory() as vault_a, tempfile.TemporaryDirectory() as vault_b:
            client = FakeClient(
                {"user-a": self.connected_settings(Path(vault_a))},
            ...
            self.assertTrue((Path(vault_a) / "Captures" / "user-a.md").exists())
            self.assertFalse((Path(vault_b) / "Captures" / "user-a.md").exists())
    ...
    def test_user_b_queue_item_uses_user_b_vault_path(self) -> None:
        with tempfile.TemporaryDirectory() as vault_a, tempfile.TemporaryDirectory() as vault_b:
            client = FakeClient(
                {
                    "user-a": self.connected_settings(Path(vault_a)),
                    "user-b": self.connected_settings(Path(vault_b)),
                },
            ...
            self.assertTrue(processed)
            self.assertTrue((Path(vault_b) / "Captures" / "user-b.md").exists())
            self.assertFalse((Path(vault_a) / "Captures" / "user-b.md").exists())
    ...
    def test_disabled_or_missing_settings_do_not_write_to_any_vault(self) -> None:
        with tempfile.TemporaryDirectory() as vault:
            client = FakeClient(
                {
                    "disabled": {
                        **self.connected_settings(Path(vault)),
                        "enabled": False,
                    },
                },
    ...
    def test_queue_target_path_cannot_escape_vault_directory(self) -> None:
        with tempfile.TemporaryDirectory() as vault:
            client = FakeClient(
                {"user-a": self.connected_settings(Path(vault))},
